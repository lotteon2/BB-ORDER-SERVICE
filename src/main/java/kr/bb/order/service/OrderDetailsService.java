...
public class OrderDetailsService {
    ...
    public OrderDeliveryGroup getOrderDetailsForUser(String orderGroupId){
        List<OrderDelivery> orderDeliveryList = orderDeliveryRepository.findByOrderGroupId(
                orderGroupId);

        // feign 요청
        List<String> productIds = orderDeliveryList.stream().flatMap(orderDelivery -> orderDelivery.getOrderDeliveryProducts().stream().map(
                OrderDeliveryProduct::getProductId)).collect(Collectors.toList());
        List<ProductInfoDto> productInfoDtos = productServiceClient.getProductInfo(productIds).getData();
        Map<String, ProductInfoDto> productInfoDtoMap = productInfoDtos.stream().collect(Collectors.toMap(ProductInfoDto::getProductId, dto -> dto));

        List<Long> storeIds = orderDeliveryList.stream().map(OrderDelivery::getStoreId).collect(
                Collectors.toList());
        Map<Long, String> storeNameMap = storeServiceClient.getStoreName(storeIds).getData();

        List<Long> deliveryIds = orderDeliveryList.stream().map(OrderDelivery::getDeliveryId).collect(
                Collectors.toList());
        Map<Long, DeliveryInfoDto> deliveryInfoMap = deliveryServiceClient.getDeliveryInfo(deliveryIds)
                .getData();
        ...
        for(OrderDelivery orderDelivery : orderDeliveryList){
            ...
            OrderInfoForStore orderInfoForStore = OrderInfoForStore.toDto(orderDelivery, productReadList,
                    storeNameMap, deliveryInfoMap);
            orderInfoForStores.add(orderInfoForStore);
        }

        // 배송정보는 일단 한 가게만 보여준다.
        DeliveryInfoDto deliveryInfoDto = deliveryInfoMap.get(deliveryIds.get(0));
        return OrderDeliveryGroup.toDto(orderGroupId, orderInfoForStores,
                paymentDate, deliveryInfoDto);
    }
}
